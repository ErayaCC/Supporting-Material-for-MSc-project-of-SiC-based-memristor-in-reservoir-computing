...
def load_data(directory, map1, map2, map3):
    ...
    for file_name in os.listdir(directory):
        if file_name.endswith(".txt"):
            ...
            currents_total = currents1 + currents2 + currents3
            label = int(re.search(r'label_(\d+)_', file_name).group(1))
            features.append(currents_total)
            labels.append(label)
    return np.array(features), np.array(labels)

X, y = load_data(r'C:\Users\97843\Desktop\MNIST_2\Processed_20', base_current_map_FC, base_current_map_FC1, base_current_map_FC2)
...
learning_rates = np.logspace(-5, -2, 30)  # Generate 30 learning rates between 1e-5 and 1e-2
...
lrs, accuracies = zip(*results)
plt.figure(figsize=(10, 5))
plt.semilogx(lrs, accuracies, marker='o', linestyle='-', color='b')
